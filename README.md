...
- 🔎 **Умное автоопределение** — начни вводить название и HailuPK сам найдёт программу и подставит путь. Поиск понимает акронимы (введи «CS2» → найдётся «Counter-Strike 2»), сканирует популярные приложения по типичным местам установки, ярлыки меню «Пуск», **игры Steam** (запуск через `steam://run` — игра реально стартует, а не открывается страница) **и игры Epic Games** (читает манифесты лаунчера и запускает .exe напрямую — например Civilization V).
- 🎮 **Кнопки запуска** — добавь любую программу, игру, ярлык, папку или ссылку (`.exe`, `steam://`, `https://`…) и запускай её одной кнопкой.
- 🧊 **Настоящие иконки + 3D-вид** — по умолчанию подставляется родная иконка приложения (извлекается из `.exe`); если её нет — выбор из готовых эмодзи или своя картинка. Плитки оформлены как объёмные «3D-иконки» с глянцем; цвет — палитра как в темах или «Авто» по названию.
...
- 🖥️ **Автозапуск с Windows** — HailuPK стартует вместе с системой.
- 🟢 **Выбор профиля при старте ПК** — при включении компьютера появляется окно «Какой профиль запустить?» с кнопками запуска (можно отключить или заменить на авто-запуск конкретного профиля без вопросов).
