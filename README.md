...
- 🔎 **Умное автоопределение** — начни вводить название и HailuPK сам найдёт программу и подставит путь. Поиск понимает акронимы (введи «CS2» → найдётся «Counter-Strike 2»), сканирует популярные приложения по типичным местам установки, ярлыки меню «Пуск» **и игры Steam** (читает библиотеку Steam, запуск через `steam://`).
